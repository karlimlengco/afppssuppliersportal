        ...
        <div class="printable-form-wrapper">
            ...
            <div class="printable-form">
                ...
                <div class="printable-form__body">
                    ...
                    <p style="text-align: justify">We are pleased to notify you that your price offer/s for the procurement of {{translateToWords($data['items'][0]->quantity)}} ({{$data['items'][0]->quantity}}) {{$data['items'][0]->item_description}} @if(count($data['items']) > 1) and {{translateToWords(count($data['items']))}} {{count($data['items']) - 1}} LI @endif only of RFQ No.{{$data['rfq_number']}} dated {{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$data['rfq_date'])->format('d F Y')}} for the use of {{$data['unit']}} in the amount of {{translateToWords($data['bid_amount'])}} pesos only (Php {{formatPrice($data['bid_amount'])}}) is hereby accepted. A contract for the procurement of this item/s is being awarded to your company/firm.</p>
