        ...
        <div class="printable-form-wrapper">
            ...
            <div class="printable-form">
                ...
                <div class="printable-form__body">
                    ...
                    <p class="indent-first-line">THIS IS TO CERTIFY that the procurement project of the {{$data['center']}}  for the procurement of {{translateToWords($data['items'][0]->quantity)}} ({{$data['items'][0]->quantity}}) {{$data['items'][0]->item_description}} @if(count($data['items']) > 1) and {{count($data['items']) - 1}} @endif only FOR THE USE OF {{$data['unit']}} was posted at conspicuous place at bulletin board of {{$data['center']}} from {{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$data['date'])->format('d F Y')}} to {{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$data['date_to'])->format('d F Y')}}</p>
